...
#public OnPlayerConnect(playerid)
{
    ...
    if (dini_Exists(path))
    {
        PlayerJob[playerid] = dini_Int(path, "Emprego");
...
CMD:pegarmecanico(playerid, params[])
{
    ...
    new path[64];
    ContaPath(playerid, path, sizeof path);
    dini_IntSet(path, "Emprego", EMPREGO_MECANICO);
...
CMD:reparar(playerid, params[])
{
    ...
    new alvo;
    if (sscanf(params, "d", alvo)) return SendClientMessage(playerid, -1, "/reparar [id]");

    if (!IsPlayerInRangeOfPoint(playerid, 5.0, OFICINA_X, OFICINA_Y, OFICINA_Z))
        return SendClientMessage(playerid, -1, "Somente na oficina.");

    new veh = GetPlayerVehicleID(alvo);
    if (!veh) return SendClientMessage(playerid, -1, "Player não está em veículo.");
